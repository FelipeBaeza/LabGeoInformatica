...
%==============================================================================
\section{Los Datos: De Dónde Viene la Información}
%==============================================================================

Para realizar este análisis, necesitábamos información geográfica detallada sobre la isla. Utilizamos la fuente de datos llamada \textbf{OpenStreetMap} (OSM) junto con IDE Chile.
...
Esto nos permitio crear mapas de densidad que muestran claramente donde hay mas y menos construcciones. Ademas, aplicamos \textbf{Analisis de Componentes Principales (PCA)} para reducir la dimensionalidad y encontrar patrones multivariados en las caracteristicas de las edificaciones.
...
Entrenamos y comparamos multiples modelos de inteligencia artificial para predecir la densidad de edificaciones. Siguiendo los requerimientos del laboratorio, evaluamos:

\begin{itemize}
    \item \textbf{Random Forest}: Ensamble de arboles de decision
    \item \textbf{XGBoost}: Gradient Boosting optimizado
    \item \textbf{SVM (Support Vector Machine)}: Modelo de kernel RBF
    \item \textbf{Gradient Boosting}: Boosting secuencial tradicional
\end{itemize}
...
\begin{itemize}
    ...
    \item Posicion geografica normalizada (X, Y)
\end{itemize}

Para la validacion, utilizamos \textbf{GroupKFold} con zonas geograficas como grupos, evitando el data leakage espacial que ocurriria con validation cruzada tradicional.

Los mejores resultados los obtuvimos con el modelo \textbf{Random Forest}, seguido de cerca por XGBoost. Ambos lograron explicar aproximadamente el 85-88\% de la variacion en la densidad edificatoria.
...
\begin{enumerate}
    \item \textbf{Concentración extrema}: Isla de Pascua tiene un patrón de desarrollo urbano muy concentrado, con casi toda la actividad en Hanga Roa
